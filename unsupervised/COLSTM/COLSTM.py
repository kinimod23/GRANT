...
def COLSTM(opt):
    ...
    if opt.pos_tags:
        pos = Input(batch_shape=(opt.batch_size, opt.seq_length), dtype='int32', name='pos')
        pos_vecs = Embedding(opt.char_vocab_size, opt.char_vec_size, input_length=opt.seq_length)(pos)
    ...
    if opt.use_chars:
        ...
        if opt.use_words and opt.pos_tags:
            x = Concatenate()([cnn, word_vecs, pos_vecs])
            inputs = [chars, word, pos]
        elif opt.use_words:
            x = Concatenate()([cnn, word_vecs])
            inputs = [chars, word]
        elif opt.pos_tags:
            x = Concatenate()([cnn, pos_vecs])
            inputs = [chars, pos]
        else:
            x = cnn
            inputs = chars
        ...
        if opt.pos_tags:
            x = Concatenate()([word_vecs, pos_vecs])
            inputs = [word, pos]
        else:
            x = word_vecs
            inputs = word
    ...
    #optimizer = SGD(lr=0.1, decay=0, momentum=0.9, nesterov=True)
