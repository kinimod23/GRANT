...
def main(opt):
    loader = BatchLoaderUnk(opt.tokens, opt.data_dir, opt.batch_size, opt.seq_length, opt.max_word_l, opt.n_words, opt.n_chars, opt.pos_tags, opt.savefile, opt.retrain)
    ...
    # define the model
    if not opt.skip_train and not opt.retrain:
        ...
        model = LSTMCNN(opt)
        model.load_weights('{}/{}.h5'.format(opt.checkpoint_dir, opt.retrain_model))
        print(model.summary())
        if opt.retrain:
            model.fit_generator(loader.next_batch(Train), loader.split_sizes[Train], opt.max_epochs,
                                loader.next_batch(Validation), loader.split_sizes[Validation], opt)
...
if __name__=="__main__":
    ...
    # data
    parser.add_argument('--data_dir', type=str, default='data/simple-wiki/', help='data directory. Should contain train.txt/valid.txt with input data')
    parser.add_argument('--savefile', type=str, default='simple-wiki-50000', help='filename to autosave the checkpoint to. Will be inside checkpoint_dir/')
    # model params
    ...
    parser.add_argument('--batch_size', type=int, default=48, help='number of sequences to train on in parallel')
    parser.add_argument('--max_epochs', type=int, default=20, help='number of full passes through the training data')
    ...
    parser.add_argument('--n_words', type=int, default=50000, help='max number of words in model')
    parser.add_argument('--n_chars', type=int, default=100, help='max number of char in model')
    parser.add_argument('--pos_tags', default=False, help='use pos tag as feature - you need a tagged corpora')
    # bookkeeping
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='unsupervised/LSTMCNN/cv', help='output directory where checkpoints get written')
    ...
    parser.add_argument('--retrain', default=False, help='additional training of trained model', action='store_true')
    parser.add_argument('--retrain_model', type=str, default='wiki-simple-50000', help='model that you want to train a bit more')
