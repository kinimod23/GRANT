...
def generate_vocabulary(data, save_vocab):
    all_data = " ".join(data)
    dd = defaultdict(int)
    words = [word.lower() for sent in sent_tokenize(all_data) for word in word_tokenize(sent)]
    for word in words:
        dd[word] += 1
    vocabulary = []
...
class MyDataset(Dataset):
    ...
    def __init__(self, df, dict_path, task, max_length_sentences=10, max_length_word=20):
    ...
    def __getitem__(self, index):
        ...
        document_encode = [
            [self.dict.index(word) if word in self.dict else -1 for word in word_tokenize(text=sentences)] for sentences in sent_tokenize(text=text)]
